...
public class WarpStorageSql extends WarpStorage {
	...
	public Location loadWarp(@NotNull String arg) {
		...
		try (PreparedStatement selectStatement = SqlManager.getConnection().prepareStatement("SELECT * FROM " + Sql.getPrefix() + "Warp")) {
	...
	public void saveWarp(@NotNull Player player, @NotNull String arg) {
		...
		try (PreparedStatement insertStatement = SqlManager.getConnection().prepareStatement("INSERT INTO " + Sql.getPrefix() + "Warp (`Warp`, `World`, `X`, `Y`, `Z`, `Yaw`, `Pitch`) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
	...
	public void clear(@NotNull String arg) {
		...
		try (PreparedStatement deleteStatement = SqlManager.getConnection().prepareStatement("DELETE FROM " + Sql.getPrefix() + "Warp WHERE `Warp`=" + "'" + arg + "'")) {
